/**
 * Rev.AI — Mock AI Analysis Service
 * Simulates Gemini API sentiment classification, theme detection,
 * and management response generation for the frontend-only build.
 *
 * When the backend is ready, replace these functions with real API calls.
 */
...
function generateResponse(sentiment, theme) {
  const templates = RESPONSE_TEMPLATES[sentiment]?.[theme] || RESPONSE_TEMPLATES[sentiment]?.experience;
 ...
 * Returns { reviewText, sentiment, theme, themeIcon, response }
 */
...
/**
 * Analyze multiple reviews with a simulated loading delay.
...
export async function analyzeReviews(reviews) {
  // Simulate API latency (300-800ms per review, capped at 3s)
  const delay = Math.min(reviews.length * 350, 3000);
...
/**
 * Get all past analyses from local storage.
 */
export function getHistory() {
  try {
    const data = localStorage.getItem('revai_history');
...
/**
 * Save analysis results to local storage history.
 */
export function saveToHistory(results) {
  const existing = getHistory();
  const updated = [...results, ...existing].slice(0, 500); // keep last 500
  localStorage.setItem('revai_history', JSON.stringify(updated));
}
...
export function clearHistory() {
  localStorage.removeItem('revai_history');
}

/**
 * Get dashboard aggregate data from history.
 */
